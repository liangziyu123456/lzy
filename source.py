...
class TCTTracks():
    ...
    def __init__(self,my_d,laser,x_rel,y_rel,z_rel,min_carrier=0):
        ...
        if self.direction in ("top","bottom"):
            ...
            self.z_step=self.r_step
            self.x_step=self.y_step=self.h_step
            def _getCarrierDensity(x,y,z):
        ...
        if isinstance(my_d,R2dDetector):
            ...
            Y2d,X2d=np.meshgrid(yArray+self.lz*z_rel,xArray+self.lx*x_rel)
            Z2d = [[]for n in range(len(Y2d))]#change
    ...
    def draw_nocarrier2D(self,x_rel,y_rel,z_rel,min_carrier):
        ...
        h = ROOT.TH2D("h","pairs of carrier generation",\
            ...
            int((self.z_max-self.z_min)/self.z_step)+1,self.z_min-0.5*self.z_step,self.z_max+0.5*self.z_step)#change
        for i in range(len(self.track_position)):
